...
cnn_yan_predict <- function(test_path, mdl_path = "../AmPEP30-CNN.mdl", without_fasta_name = F, check_len = T) {
...
rf_yan_predict <- function(test_path, mdl_path = "/home/axpep/AxPEP_WWW_RUNS/Deep-AmPEP30/AmPEP30-RF.mdl", without_fasta_name = F, check_len = T) {
  rf.mdl <- readRDS(mdl_path)
  test_data <- totally_psekraac_best5(test_path, test = T, without_fasta_name = without_fasta_name, check_len = check_len)
  sco <- predict(rf.mdl, newdata = test_data, type = "vote")
  pre <- rep(1, nrow(sco))
  pre[which(sco[, 2] < 0.5)] <- 0
  return(list(sco = sco, pre = pre, seq.name = rownames(test_data)))
}
...
develop_cnn_mdl_yan <- function(rf = F) {
  ...
  if (!rf) {
    ...
    save_model_hdf5(cb$mdl, "/home/axpep/AxPEP_WWW_RUNS/Deep-AmPEP30/AmPEP30-CNN.mdl")
  } else {
    rf <- rf_yan(data, test_data = data[1:10, ], mtry = 1, ntree = 500)
    saveRDS(rf$mdl, "/home/axpep/AxPEP_WWW_RUNS/Deep-AmPEP30/AmPEP30-RF.mdl")
  }
}
...
seq.name <- read_seq(test_name = "input.fasta", check_len = T)
if (is.null(seq.name)) {
  seq.name <- read_seq(test_name = "input.fasta", check_len = F)
  name <- seq.name$name
  pre <- rep(-1, length(name))
  prob <- data.frame("seq_name" = name, "class" = pre, "AMP_probablity" = pre)
} else {
  rs <- cnn_yan_predict("input.fasta", check_len = T)
  prob <- data.frame("seq_name" = rs$seq.name, "class" = rs$pre, "AMP_probablity" = sprintf("%1.6f", rs$sco))
}
write.table(prob, "./deepampep30.out", sep = " ", col.names = F, row.names = F, quote = FALSE)
# predict_c_grabarta()
